...
DECLARE cur1 CURSOR FOR 
    SELECT id, figure_id, header, content, url, doc_id_id FROM scin_db.scin_pub_figure;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET cur1_done = TRUE;

OPEN cur1;
cur1_loop: LOOP
FETCH FROM cur1 INTO v_figure_id, v_doc_figure_id, v_header, v_content, v_url, v_doc_id_id;
    ...
    OPEN cur2;
    cur2_loop: LOOP
    FETCH FROM cur2 INTO v_tech_id, v_parental_name, v_alternative;
      ...
      IF EXISTS (SELECT 1
         ...
         INSERT INTO scin_db.pub_figure_tech_result (figure_id, doc_figure_id, header, content, url, doc_id_id, tech_id, technique_group, parental_name, alternative)
           VALUES (v_figure_id, v_doc_figure_id, v_header, v_content, v_url, v_doc_id_id, v_tech_id, '', v_parental_name, v_alternative)
           ;
      END IF;
      ...
      IF EXISTS (SELECT 1
                ...
                WHERE doc_id = v_doc_id_id
                AND header = CONCAT('S' ,v_doc_figure_id ,' Figure.')
                AND CONTENT LIKE CONCAT('%', v_alternative, '%')) THEN
                    
         INSERT INTO scin_db.pub_tech_prod_temp (figure_id, doc_figure_id, header, content, url, doc_id_id, tech_id, technique_group, parental_name, alternative)
            VALUES (v_figure_id, v_doc_figure_id, CONCAT('S' ,v_doc_figure_id ,' Figure.'), v_content, v_url, v_doc_id_id, v_tech_id, '', v_parental_name, v_alternative)
           ;
      END IF;
      */
